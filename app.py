    ...
    if lista_cants and st.session_state.piezas_dict and sum(lista_cants) > 0:
        ...
        for q_n in lista_cants:
            ...
            for p_id, p in st.session_state.piezas_dict.items():
                ...
                c_cd = (hc_d * m2 * (p.get('gd',0)/1000) * db["cartoncillo"][p.get('pd','Ninguno')]["precio_kg"])
                c_pla, c_peg = 0.0, 0.0
                
                if p["pl"] != "Ninguna":
                    c_pla = hp * m2 * db["planchas"][p["pl"]][p.get('ap','C/C')]
                    pas = (1 if p["pf"]!="Ninguno" else 0) + (1 if p.get('pd','Ninguno')!="Ninguno" else 0)
                    c_peg = hp * m2 * db["planchas"][p["pl"]]["peg"] * pas
                elif p["pf"] != "Ninguno" and p.get('pd', 'Ninguno') != "Ninguno":
                    c_peg = hp * m2 * db["planchas"]["Microcanal / Canal 3"]["peg"] * 1
