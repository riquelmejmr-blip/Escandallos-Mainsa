...
# --- 1. CONFIGURACIÓN INTERNA Y BASE DE DATOS ---
MARGEN_COMERCIAL = 2.2
IMPORTE_FIJO_VENTA_PVP = 500.0
PRECIO_HORA_MANIPULACION = 18.0
...
with st.sidebar:
    ...
    st.session_state.com = st.text_input("Nº de Comercial", st.session_state.get('com', ""))
    ...
    nombre_archivo_final = "PV_" + "_".join([str(p).strip() for p in partes_nombre if str(p).strip()]) + ".json"
    
    datos_exp = {
        "brf": st.session_state.brf, "cli": st.session_state.cli, "com": st.session_state.com, "ver": st.session_state.ver,
        "des": st.session_state.des, "piezas": st.session_state.piezas_dict, "extras": st.session_state.lista_extras_grabados, 
...
if not modo_comercial:
        ...
        with st.expander(f"🛠 {p['nombre']} - {p['h']}x{p['w']} mm", expanded=True):
            ...
            with col3:
                ...
                if p['cor'] == "Troquelado": 
                    ...
                    p['pv_troquel'] = st.number_input("PV Troquel (€)", value=float(p.get('pv_troquel', 0.0)), key=f"pvt_{p_id}")
                if p['pd'] != "Ninguno":
    ...
    if st.button("➕ Añadir Lista") and ex_sel != "---": st.session_state.lista_extras_grabados.append({"nombre": ex_sel, "coste": PRECIOS["extras_base"][ex_sel], "cantidad": 1.0}); st.rerun()
    for i, ex in enumerate(st.session_state.lista_extras_grabados):
        ca, cb = st.columns([4, 1]); ca.write(f"**{ex['nombre']}**"); ex['cantidad'] = ca.number_input("Cant", value=float(ex['cantidad']), key=f"exq_{i}")
...
if lista_cants and st.session_state.piezas_dict and sum(lista_cants) > 0:
    ...
    for q_n in lista_cants:
        ...
        for p in st.session_state.piezas_dict.values():
            ...
            c_imp = (nb*m2*6.5 if p["im"]=="Digital" else (f_o(nb)*(p.get('nt',0)+(1 if p.get('ba') else 0)) if p["im"]=="Offset" else 0))
        ...
        c_mo = ((seg_man_total/3600)*18*qp_taller) + (qp_taller*DIFICULTAD_ESTANDAR)
        
        # Cálculo Embalaje (Margen incluido + Unitario)
        coste_embalaje_taller = sum(motor_embalajes(em['tipo'], em['l'], em['w'], em['a'], q_n*em['uds']) * (q_n*em['uds']) for em in st.session_state.lista_embalajes)
        pv_embalaje_total = coste_embalaje_taller * MARGEN_COMERCIAL
        pv_embalaje_unitario = pv_embalaje_total / q_n if q_n > 0 else 0
        
        # PVP PRODUCTO (Fabricación * Margen + 500 Fijos)
        pvp_producto_base = ((coste_f + c_ext_tot + c_mo) * MARGEN_COMERCIAL) + IMPORTE_FIJO_VENTA_PVP
        
        # SUMA TOTAL OPERACIÓN
        pvp_total_todo = pvp_producto_base + pv_embalaje_total + total_pv_troqueles
        
        res_final.append({
            "Cant": f"{q_n} uds", 
            "Producto (Total)": f"{pvp_producto_base:.2f}€",
            "Embalaje (Ud)": f"{pv_embalaje_unitario:.3f}€",
...
if modo_comercial and res_final:
    ...
    f_h = "".join([f"<tr><td>{r['Cant']}</td><td>{r['Producto (Total)']}</td><td>{r['Embalaje (Ud)']}</td><td>{r['Troqueles']}</td><td class='total-row'>{r['TOTAL OPERACIÓN']}</td></tr>" for r in res_final])
    st.markdown(f"""<div class="comercial-box"><h2 class="comercial-header">OFERTA COMERCIAL - {st.session_state.cli}</h2><p><b>Referencia:</b> {st.session_state.brf} | <b>Descripción:</b> {st.session_state.des}</p><h4>Especificaciones</h4><ul>{p_html}</ul><table class="comercial-table"><tr><th>Cantidad</th><th>Producto (Total)</th><th>Embalaje (Unitario)</th><th>Troqueles (Total)</th><th>TOTAL OPERACIÓN</th></tr>{f_h}</table></div>""", unsafe_allow_html=True)
else:
    if res_final:
