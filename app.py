...
def motor_embalajes(tipo, largo, ancho, alto, q):
    """Lógica estadística para cajas Canal 5"""
    if q <= 0: return 0.0
...
# --- 5. GESTIÓN DE FORMAS, EXTRAS Y EMBALAJES ---
if not modo_comercial:
    ...
    # SECCIÓN EMBALAJES (NUEVO)
    st.divider(); st.subheader("📦 Complemento de Embalajes")
    col_em1, col_em2 = st.columns([2, 1])
    tipo_em = col_em1.selectbox("Seleccionar Tipo:", ["Plano (Canal 5)", "En Volumen (Pendiente)", "Guainas (Pendiente)"])
...
if lista_cants and st.session_state.piezas_dict and sum(lista_cants) > 0:
    for q_n in lista_cants:
        ...
        for em in st.session_state.lista_embalajes:
            total_q_cajas = q_n * em['uds']
            u_coste = motor_embalajes(em['tipo'], em['l'], em['w'], em['a'], total_q_cajas)
            c_em_tot += u_coste * total_q_cajas
...
if modo_comercial and res_final:
    ...
    for p in st.session_state.piezas_dict.values():
        ...
        p_html += f"<li><b>{p['nombre']}:</b> {p['h']}x{p['w']} mm ({p['pliegos']:g} pl/ud)<br/>&nbsp;&nbsp;&nbsp;• Cara: {p['pf']} ({p.get('gf',0)}g) | Imp: {p['im']} | Acabado: {ac_c_f}</li>"
    ex_h_list = [f"<li>{e['nombre']} (x{e['cantidad']:g})</li>" for e in st.session_state.lista_extras_grabados]
    for em in st.session_state.lista_embalajes: ex_h_list.append(f"<li>Caja {em['tipo']} {em['l']}x{em['w']}x{em['a']} (x{em['uds']})</li>")
    ex_h = "".join(ex_h_list) if ex_h_list else "<li>Sin accesorios ni cajas</li>"
    f_h = "".join([f"<tr><td>{r['Cant']} uds</td><td>{r['Total']}</td><td><b>{r['Ud']}</b></td></tr>" for r in res_final])
    st.markdown(f"""<div class="comercial-box"><h2 class="comercial-header">OFERTA COMERCIAL - BRIEFING {st.session_state.brf}</h2><div class="header-info"><span><b>Cliente:</b> {st.session_state.cli}</span><span><b>Versión:</b> {st.session_state.ver}</span></div><p><b>Descripción:</b> {st.session_state.des}</p><h4>Especificaciones</h4><ul>{p_html}</ul><h4>Accesorios y Embalaje</h4><ul>{ex_h}</ul><table class="comercial-table"><tr><th>Cantidad</th><th>PVP Total</th><th>PVP Unitario</th></tr>{f_h}</table></div>""", unsafe_allow_html=True)
else:
    if res_final:
        ...
        for q, info in desc_full.items():
            with st.expander(f"🔍 Auditoría {q} uds"):
                ...
                c1.metric("Mano Obra", f"{info['mo']:.2f}€"); c2.metric("Extras+Emb", f"{info['extras']:.2f}€"); c3.metric("Fijo", f"{info['fijo']:.2f}€"); c4.metric("TOTAL FAB.", f"{info['total']:.2f}€")
