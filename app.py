...
# Lógica del motor de embalajes (Prompt GPT)
...
with st.sidebar:
    ...
    datos_exp = {
        ...
        "embalajes": st.session_state.lista_embalajes, "imp_fijo": imp_fijo
    }
...
if lista_cants and st.session_state.piezas_dict and sum(lista_cants) > 0:
    for q_n in lista_cants:
        ...
        for p_id, p in st.session_state.piezas_dict.items():
            ...
            largo_p, ancho_p = p['h'], p['w']
            if largo_p > 1000 or ancho_p > 700: v_arr, v_tir = 107.80, 0.135
            elif largo_p < 1000 and ancho_p < 700: v_arr, v_tir = 48.19, 0.06
        ...
        desc_full[q_n] = {"det": det_f, "mo": c_mo, "extras": c_ext_tot + c_em_tot, "fijo": imp_fijo, "total": t_fab, "qp": qp_taller}
...
if modo_comercial and res_final:
    ...
    for em in st.session_state.lista_embalajes: ex_h_list.append(f"<li>Embalaje {em['tipo']} {em['l']}x{em['w']}x{em['a']} (x{em['uds']})</li>")
    ...
    if res_final:
        ...
        for q, info in desc_full.items():
            with st.expander(f"🔍 Auditoría Compras {q} uds (Taller: {info['qp']} uds)"):
                ...
                c1.metric("Mano Obra", f"{info['mo']:.2f}€"); c2.metric("Extras+Emb", f"{info['extras']:.2f}€"); c3.metric("Imp. Fijo", f"{info['fijo']:.2f}€"); c4.metric("TOTAL FAB.", f"{info['total']:.2f}€")
