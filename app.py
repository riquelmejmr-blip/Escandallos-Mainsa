...
# --- 1. BASE DE DATOS DE PRECIOS (Valores exactos de tu imagen) ---
PRECIOS = {
    "cartoncillo": {
...
with st.sidebar:
    ...
    cants_str = st.text_input("Cantidades (ej: 200, 500)", "200, 500")
...
# --- 4. CONFIGURACIÓN DE PIEZAS Y EXTRAS ---
extras_def_datos = []
if not modo_comercial:
    ...
    extras_pred_lista = st.multiselect("Accesorios estándar:", list(PRECIOS["extras_precios"].keys()))
    if extras_pred_lista:
        cols_ex = st.columns(len(extras_pred_lista))
        for j, ex_n in enumerate(extras_pred_lista):
            cant_ex = cols_ex[j].number_input(f"Uds {ex_n}", 1.0, key=f"q_ex_{j}")
            extras_def_datos.append({"nombre": ex_n, "coste": PRECIOS["extras_precios"][ex_n], "cantidad": cant_ex})

    st.markdown("**Extras Manuales:**")
    ce1, ce2, ce3 = st.columns([3, 2, 1])
    e_nom = ce1.text_input("Nombre (ej: Bolsa)", key="ex_m_n")
    e_cost = ce2.number_input("Coste Ud (€)", 0.0, 100.0, 0.0, key="ex_m_c")
    if ce3.button("➕"): 
        if e_nom: st.session_state.extras_manuales.append({"nombre": e_nom, "coste": e_cost, "cantidad": 1.0}); st.rerun()
...
if lista_cants and st.session_state.piezas_dict:
    for q_n in lista_cants:
        ...
        for p_id, p in st.session_state.piezas_dict.items():
            ...
            sub = c_mat + c_pla + c_imp + c_acab + c_trq
            coste_f += sub
            det_f.append({"Pieza": p["nombre"], "Mat": c_mat, "Plancha": c_pla, "Imp": c_imp, "Acab": c_acab, "Corte": c_trq, "Total": sub})

        c_ext_pred = sum(e["coste"] * e["cantidad"] * qp_taller for e in extras_def_datos)
        c_ext_man = sum(e["coste"] * e["cantidad"] * qp_taller for e in st.session_state.extras_manuales)
        c_man = ((seg_man/3600)*18*qp_taller) + (qp_taller*dif_ud) + c_ext_pred + c_ext_man
...
if modo_comercial:
    ...
    ex_nombres = [e['nombre'] for e in extras_def_datos] + [e['nombre'] for e in st.session_state.extras_manuales]
    ...
    if res_final:
        ...
        for q, info in desc_full.items():
            with st.expander(f"🔍 Desglose {q} uds"):
                ...
                st.write(f"**Manipulación y Extras:** {info['man']:.2f}€ | **Total Fab:** {info['total']:.2f}€")
