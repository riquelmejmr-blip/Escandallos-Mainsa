...
# --- 3. CONFIGURACIÓN GLOBAL SUPERIOR ---
modo_comercial = st.sidebar.checkbox("VISTA OFERTA COMERCIAL", value=False)
...
if not modo_comercial:
    ...
    cants_str = c_c1.text_input("Cantidades (separadas por comas)", "0")
    ...
    # Definición de respaldo para que el motor no falle en modo comercial
    lista_cants = []
    seg_man_total = 0
    dif_ud = 0.091
    margen = 2.2
...
if not modo_comercial:
    ...
    for p_id in list(st.session_state.piezas_dict.keys()):
        ...
        with st.expander(f"🛠 {p['nombre']}", expanded=True):
            ...
            with col2:
                ...
                if p['pf'] != "Ninguno": p['gf'] = st.number_input("Gramaje F.", value=int(p.get('gf', 0)), key=f"gf_{p_id}")
    ...
    with ce2:
        en = st.text_input("Extra Manual:", key="ex_m_n"); ec = st.number_input("Coste Ud:", 0.0, 50.0, key="ex_m_c"); eq = st.number_input("Cant/ud:", 1.0, 100.0, key="ex_m_q")
...
# --- 5. MOTOR DE CÁLCULO ---
res_final, desc_full = [], {}
# Redefinimos lista_cants por seguridad antes del bloque
if not modo_comercial:
...
# --- 6. SALIDA VISUAL ---
if modo_comercial:
    # Forzar el renderizado HTML correcto
    p_lines = []
    for p in st.session_state.piezas_dict.values():
        ac_c = []
        if p.get('pel') and p['pel'] != "Sin Peliculado": ac_c.append(p['pel'])
        if p['im'] == "Offset" and p.get('ba'): ac_c.append("Barniz")
        if p['im'] == "Digital" and p.get('ld'): ac_c.append("Laminado")
        line = f"<li><b>{p['nombre']}:</b> {p['w']}x{p['h']} mm<br/>"
        line += f"&nbsp;&nbsp;&nbsp;• Cara: {p['pf']} ({p.get('gf',0)}g) | Imp: {p['im']} | Acabado: {' + '.join(ac_c) if ac_c else 'Sin acabado'}<br/>"
        ...
        if p.get('pd') and p['pd'] != "Ninguno":
            ...
            if p.get('im_d') == "Digital" and p.get('ld_d'): ac_d.append("Laminado")
            line += f"&nbsp;&nbsp;&nbsp;• Dorso: {p['pd']} ({p.get('gd',0)}g) | Imp: {p['im_d']} | Acabado: {' + '.join(ac_d) if ac_d else 'Sin acabado'}"
        line += "</li>"; p_lines.append(line)
    ex_h = "".join([f"<li>{e['nombre']} (x{e['cantidad']})</li>" for e in st.session_state.lista_extras_grabados])
    
    # Re-calculamos res_final solo para la vista comercial si está vacío
    # (En un uso real, el usuario primero mete datos y luego cambia de vista)
    f_h = ""
    ...
    st.markdown(f"""<div class="comercial-box">
        ...
        {"<p><b>Descripción:</b> " + st.session_state.des + "</p>" if st.session_state.des else ""}
        <h4>1. Especificaciones</h4><ul>{"".join(p_lines)}</ul>
        <h4>2. Accesorios y Montaje</h4><ul>{ex_h}<li>Montaje especializado incluido.</li></ul>
